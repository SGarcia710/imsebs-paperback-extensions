...
var _Sources = (() => {
  ...
  var URLBuilder = class {
    ...
    buildUrl() {
      ...
      const query = this.queryPairs.map(([key, value]) => `${encodeURIComponent(key)}=${encodeURIComponent(value)}`).join("&");
  ...
  function toArrayData(responseData) {
    ...
    if (responseData != null && typeof responseData.toString === "function") {
  ...
  function rc4Bytes(key, input) {
    ...
    for (let y = 0; y < input.length; y++) {
      ...
      const k = state[state[ii] + state[j] & 255];
  ...
  async function parseSearchOptions(source) {
    ...
    const $2 = load(toArrayData(response.data));
    const parseDropdownOptions = (selector) => {
      const items = [];
      $2(selector).each((_idx, node) => {
        const value = $2(node).find("input").attr("value") ?? "";
        const label = $2(node).find("label").text().trim();
        if (value && label) {
  ...
  async function parseSearchResultsPage(source, url) {
    ...
    const $2 = load(toArrayData(response.data));
    const items = [];
    $2(".original.card-lg .unit .inner").each((_idx, node) => {
      const item = $2(node);
      const link = item.find(".info > a").first();
      const title = link.text().trim();
      const mangaId = parseMangaIdFromUrl(link.attr("href"));
      const image = item.find("img").attr("src") ?? "";
      const chapterMatch = item.find(".content[data-name='chap'] a").first().find("span").first().text().trim().match(/Chap\s+([\d.]+)/i);
  ...
  function parseFilterTiles($2) {
    const results = [];
    $2(".unit .inner").each((_idx, node) => {
      const item = $2(node);
  ...
  var MangaFire = class _MangaFire {
    constructor() {
...
Please go to the homepage of <${_MangaFire.name}> and press the cloud icon.`
    ...
    async getSearchResults(query, metadata) {
      ...
      const builder = new URLBuilder(MF_DOMAIN).addPathComponent("filter").addQueryParameter("keyword", title).addQueryParameter("page", String(page)).addQueryParameter("genre_mode", "and").addQueryParameter("vrf", generateVRF(title));
    ...
    async getMangaDetails(mangaId) {
      ...
      const $2 = load(toArrayData(response.data));
      const title = $2(".manga-detail .info h1").first().text().trim();
      const secondary = $2(".manga-detail .info h6").first().text().trim();
      const image = $2(".manga-detail .poster img").attr("src") ?? "";
      const synopsis = $2("#synopsis .modal-content").text().trim() || $2(".manga-detail .info .description").text().trim() || "";
      const authors = [];
      $2("#info-rating .meta div").each((_idx, node) => {
        const sectionName = $2(node).find("span").first().text().trim();
        if (sectionName !== "Author:") return;
        $2(node).find("a").each((_aIdx, authorNode) => {
          const author = $2(authorNode).text().trim();
          if (author) authors.push(author);
        });
      });
      let rawStatus = "";
      $2(".manga-detail .info p").each((_idx, node) => {
        const text3 = $2(node).text().trim();
        if (text3) rawStatus = text3;
      });
      const genreLabels = [];
      $2("#info-rating .meta div").each((_idx, node) => {
        const sectionName = $2(node).find("span").first().text().trim();
        if (sectionName !== "Genres:") return;
        $2(node).find("a").each((_gIdx, genreNode) => {
          const genre = $2(genreNode).text().trim();
    ...
    async getChapters(mangaId) {
      ...
      for (const language of SUPPORTED_LANGUAGES) {
        ...
        try {
          ...
          const chapterReferenceHtml = typeof readJson?.result === "string" ? readJson.result : readJson?.result?.html ?? "";
          if (!chapterReferenceHtml) {
            continue;
          }
          const reference$ = load(chapterReferenceHtml);
          const chapterIdByNumber = /* @__PURE__ */ new Map();
          ...
          const detailsHtml = typeof detailsJson?.result === "string" ? detailsJson.result : detailsJson?.result?.html ?? "";
          if (!detailsHtml) {
            continue;
          }
          const detail$ = load(detailsHtml);
          detail$("li").each((_idx, node) => {
            ...
            const cleanedTitle = firstSpanText.split(`${chapterNumberRaw}:`)[1]?.trim() || firstSpanText.replace(/Chap\s+[\d.]+/i, "").trim() || void 0;
    ...
    async getChapterDetails(mangaId, chapterId) {
      ...
      const response = await this.requestManager.schedule(request, 1);
      this.CloudFlareError(response.status);
      const data2 = JSON.parse(toArrayData(response.data));
      const images = data2?.result?.images ?? [];
      const pages = images.map((entry) => Array.isArray(entry) ? entry[0] : void 0).filter((url) => !!url);
    ...
    async getHomePageSections(sectionCallback) {
      ...
      const popularUrl = new URLBuilder(MF_DOMAIN).addPathComponent("filter").addQueryParameter("keyword", "").addQueryParameter("language[]", "en").addQueryParameter("sort", "most_viewed").addQueryParameter("page", "1").buildUrl();
      const popularPage = await parseSearchResultsPage(this, popularUrl);
      popularSection.items = popularPage.items;
      sectionCallback(popularSection);
      const updatedUrl = new URLBuilder(MF_DOMAIN).addPathComponent("filter").addQueryParameter("keyword", "").addQueryParameter("language[]", "en").addQueryParameter("sort", "recently_updated").addQueryParameter("page", "1").buildUrl();
      ...
      const added$ = load(toArrayData(addedResponse.data));
      const addedItems = parseFilterTiles(added$).map(
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      if (homepageSectionId === "new_manga_section") {
        ...
        this.CloudFlareError(response.status);
        const $2 = load(toArrayData(response.data));
        const parsed = parseFilterTiles($2).map(
          (item) => App.createPartialSourceManga({
            mangaId: item.mangaId,
            image: item.image,
            title: item.title,
        ...
        const hasNext = !!$2(".page-item.active + .page-item .page-link").length;
        return App.createPagedResults({
          results: parsed,
