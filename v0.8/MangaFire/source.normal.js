...
// Reverse-engineered and adapted from MangaFire Paperback v0.9 source.
// Target style: Paperback v0.8 extension shape used in this repository.
...
function loadHtml(html, options) {
  if (cheerioLoad) return cheerioLoad(html, options);
  if (typeof load === "function") return load(html, options);
  throw new Error("No cheerio loader available. Install 'cheerio' or provide global load().");
}
...
const MangaFireInfo = {
  ...
  intents:
    PBSourceIntents.MANGA_CHAPTERS |
    PBSourceIntents.HOMEPAGE_SECTIONS |
    PBSourceIntents.CLOUDFLARE_BYPASS_REQUIRED |
    PBSourceIntents.SETTINGS_UI,
  sourceTags: []
};
...
function rc4Bytes(key, input) {
  ...
  for (let y = 0; y < input.length; y++) {
    ...
    j = (j + state[i]) & 0xff;
    [state[i], state[j]] = [state[j], state[i]];
    const k = state[(state[i] + state[j]) & 0xff];
...
async function parseSearchOptions(source) {
  ...
  const $ = loadHtml(toArrayData(response.data));
...
class MangaFire {
  ...
  async getChapters(mangaId) {
    ...
    for (const language of SUPPORTED_LANGUAGES) {
      ...
      try {
        ...
        const reference$ = loadHtml(chapterReferenceHtml);
        ...
        const detail$ = loadHtml(detailsHtml);
  ...
  async getChapterDetails(mangaId, chapterId) {
    ...
    const pages = images.map((entry) => entry[0]).filter(Boolean);
  ...
  async getHomePageSections(sectionCallback) {
    ...
    const added$ = loadHtml(toArrayData(addedResponse.data));
...
const Sources = { MangaFire, MangaFireInfo };

if (typeof module !== "undefined" && module.exports) {
  module.exports = { Sources, MangaFire, MangaFireInfo };
}
